    ...
    */
    ...
    for ( int i = 0; i < 20; i++ ) {
        ...
        if ( (valid == 1) && (temp == 1)){ // queue up block if 1
            ...
            scanf_s("%200c",msg);
			*/
			string txhash, chash, vhash;
			
			int c;
			cout << "Select Candidate to caste a vote."<<endl<<" Press 1 for PPP, Press 2 for PTI, Press 3 for PMLN,"
<<
				endl<< "Press 4 for Jamat e Islami & Press 5 for JUI " <<  endl;            
			cin >> c;
			switch(c)
			{
			case 1:
				chash = sha256(" Asif Ali Zardari");
				cout << "PPP CHASH: " << chash << endl;
				break;
			case 2:
				chash = sha256(" Imran Khan");
				cout << "PTI CHASH: " << chash << endl;
				break;
			case 3:
				chash = sha256("Nawaz Sharif ");
				cout << "PMLN CHASH: " << chash << endl;
				break;
			case 4:
				chash = sha256("Siraj ul Haq ");
				cout << "Jamat e Islami CHASH: " << chash << endl;
				break;
			case 5:
				chash = sha256("Molana Fazl ur Rehman");
				cout << "JUI CHASH: " << chash << endl;
				break;
			}			
			cout << "Vhash is: " << a1 << endl;	
			string s2;
		s2 = sha256(chash+a1 );
		cout << "txhash is: " << s2 << endl;
			string str = bc.addTx(s2, chash, a1);
			v.push_back(str);
